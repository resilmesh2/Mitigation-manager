...
def test_attack_graph_linkage():
    ...
    cur = node._next
    prev = node
    assert prev._prev is None
    while cur is not None:
        assert prev._next is cur
        assert cur._prev is prev
        prev = cur
        cur = cur._next
    assert prev._next is None
...
def test_attack_graph_sets():
    ...
    node = node._next  # pyright:ignore
...
def test_factor_2():
    ...
    node.last()._prev._next = None  # pyright: ignore
    ...
    node.last()._next = AttackNode('T9998', []).then(AttackNode, 'T9999', []).first()
