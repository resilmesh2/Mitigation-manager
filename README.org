...
The AI planning engine builds mitigations through a combination of attack
graphs, detailed conditions, CoA playbooks encoded into workflows, and data
encoded in the alert.  In order to be efficient across a wide range of
scenarios, these aspects can all be configured through the included Extensive
Data Notation (EDN) files located inside the =data/= directory.

The EDN files included in the repository contain a series of examples used
during development, together with real data used during Resilmesh pilots.
Detailed descriptions of their contents and how to populate them are found
below:

** Alert field mappings

Located inside =data/field-mappings.edn=, this file contains a vector of Clojure
maps used to extract information from incoming alerts.  Each map is used as a
mapping data structure that mirrors the shape of the input JSON:

- If a value in the map is another map, traversal continues recursively.

- If a value in the map is a keyword, that keyword becomes the field name in the
  flattened output.

- The corresponding value from the input JSON is extracted and placed in the
  output under the new field name.

- Any fields not present in the mapping are ignored.

The result is a flexible data structure able to parse arbitrary JSON alerts and
extract known valuable fields in a deterministic and reliable fashion.  These
fields are then used when running attack graph conditions, and to initialize
workflow parameters.

** Queries

To extract useful information and verify that requirements are met, we use
"queries".  A query is a Clojure keyword associated to a vector, representing
the name/contents of the query. A contents vector will always contain one
keyword (the "type" of the query) and a second value that changes based on the
type:

- If the type is =:static=, the value is a JSON-encodable primitive.

- If the type is =:alert=, the value is a keyword.  This keyword is then used to
  extract the actual value from the map structure obtained during the alert
  field mapping process.

- If the type is =:ctx=, the value is a keyword.  This keyword is then used to
  extract the actual value from the currently evaluated attack's context.  This
  context will be further explained in subsequent sections.

- If the type is =:eval=, the value is a Clojure sequence that will be evaluated
  at runtime.  This sequence will have access to both ~*alert*~ and ~*ctx*~, two
  variables containing the aforementioned alert and attack context maps.

** Workflows

Located inside =data/workflows.edn=, this file contains a vector of Clojure maps
representing the available workflows.  Each workflow contains a textual
description, a URL to make a POST request to (generally populated with Shuffle
webhook endpoint URLs), an execution cost, an associated MITRE ATT&CK Technique
ID, and a map of queries used to initialize the workflow parameters.

** Attack graphs

Located inside =data/attack-graphs.edn=, this file contains a vector of known
attack patterns.  Each attack pattern consists of a UUID identifier, a
description, an orderless sequence of nodes, and a reference to the initial
node.

Each node contains its own UUID identifier, a textual description, a sequence of
associated MITRE ATT&CK Technique IDs, references to both previous and next
nodes, a map of values to extract (in the form of queries), and a map of
conditions (in the form of queries).  These conditions are used in order to
guarantee that a certain attack graph node has been confirmed to have occurred.
Conditions are considered to be fulfilled as long as the associated query
returns non-nil (note that, as a consequence, values such as ~false~ or ~'()~
will be treated as the condition being fulfilled).

Once a node's conditions are verified and the node is considered "triggered", it
will extract a series of values using queries, which will then be stored in the
attack instance's context (ctx for short).  This allows an attack graph to
define valuable pattern-specific data to be remembered for current and future
mitigations.
